...
Preprocessing (all optional, toggle + parameters per image):
  - brightness enhancement
  - negation
  - rescaling (to bring two images to matching resolution)
  - rotation (clockwise, arbitrary angle)
  - flip/mirror
Applied in that order: enhance -> negate -> rescale -> rotate -> flip.
"""
...
ROTATE_IMG1 = False
ROTATE_ANGLE_IMG1 = 0.0
ROTATE_IMG2 = True
ROTATE_ANGLE_IMG2 = 90

# Optional: flip/mirror an image before detection.
# FLIP_CODE: 1 = along vertical axis (left-right),
#            0 = flip along horizontal axis (upside-down), -1 = both.
FLIP_IMG1 = False
FLIP_IMG2 = True
FLIP_CODE = 1
...
def rescale_image(image, factor):
    ...
    # Gaussian pre-filter: sigma scales with how much detail is being
    # discarded (larger reduction -> stronger blur needed to band-limit).
    sigma = (1.0 / factor - 1.0) / 2.0
    ksize = max(3, int(2 * round(3 * sigma) + 1))  # odd kernel size covering ~3 sigma
...
def describe_preprocessing(enhance_flag, negate_flag, rescale_flag, rotate_flag,
                            flip_flag, enhance_factor, rescale_sizes, rotate_angle):
    """
    Human-readable record of preprocessing applied (in the order it
    actually runs: enhance -> negate -> rescale -> rotate -> flip), for
    the summary file.
    ...
    if rescale_flag:
        ...
        steps.append(f"rescaled (original resolution {ow}x{oh}, downscaled to {nw}x{nh}), gaussian filter applied before")
    if rotate_flag:
...
def describe_superpoint_downscale(sp_result):
    ...
    resolution), for the visualization titles and summary file.
    """
...
def main():
    ...
    # Rotate clockwise where configured
    if ROTATE_IMG1:
    ...
    img1_preprocessing = describe_preprocessing(
        ENHANCE_IMG1, NEGATE_IMG1, RESCALE_IMG1, ROTATE_IMG1, FLIP_IMG1,
        ENHANCE_FACTOR, rescale_sizes_1, ROTATE_ANGLE_IMG1
    )
    img2_preprocessing = describe_preprocessing(
        ENHANCE_IMG2, NEGATE_IMG2, RESCALE_IMG2, ROTATE_IMG2, FLIP_IMG2,
        ENHANCE_FACTOR, rescale_sizes_2, ROTATE_ANGLE_IMG2
    )
