...
def setup_paths():
    ...
    images_dir1 = project_root / 'data' / 'Xenium' / 'Lung'
    images_dir2 = project_root / 'data' / 'H&E' / 'Lung'
    repos_dir = project_root.parent.parent / '10. Sem' / 'Praktikum MDC' / 'git'
    results_dir = project_root / 'Results' / 'Python' / 'SIFTvsSuperPoint_same_HE_DAPI'  
...
IMAGE1_NAME = 'lungTMA_1.tif'   # e.g. "sample_A.tif"
IMAGE2_NAME = 'lungTMA_1.jpg'   # e.g. "sample_B.tif"

# Negate an image's intensities (background -> white, nuclei/tissue -> black
# for that image) before feature detection. Useful when matching a DAPI scan
# (dark background, bright nuclei) against an H&E image (light background) -
# negate the DAPI side only, leave H&E as-is.
NEGATE_IMG1 = True
NEGATE_IMG2 = False
...
# pull faint nuclei signal up out of the noise floor.
ENHANCE_IMG1 = True
ENHANCE_IMG2 = False
ENHANCE_FACTOR = 3   # >1 brightens, <1 darkens, 1 = no change

# Local adaptive contrast enhancement (CLAHE) as an alternative/addition to
# flat brightness enhancement above - better at pulling out faint signal
...
def enhance_image(image, factor=3):
    """
    Enhances image brightness (e.g. for low-contrast DAPI signal).
...
def apply_clahe(image, clip_limit=3.0, tile_grid_size=(8, 8)):
    """
    Local adaptive contrast enhancement (CLAHE), applied as a real
    preprocessing step before feature detection (not just for display).
    Unlike a flat brightness multiply, this stretches contrast separately
    per local tile, so it holds up better under uneven illumination across
    a large scan.
    """
...
# Visualization
# ============================================================================
def create_output_dir(suffix=""):
    """Create timestamped output directory inside Results/Python/, optionally
    with an extra suffix appended (e.g. '_DAPIrotated')."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    folder_name = f"{timestamp}{suffix}"
    output_dir = paths['results_dir'] / folder_name
...
def draw_matches(img1, img2, kp1, kp2, matches, inliers, title, save_path):
    ...
    display-capped resolution (VIS_MAX_DIM). img1/img2 already reflect
    whatever preprocessing was applied before detection.
    """
...
def main():
    ...
    # Brightness enhancement (e.g. weak DAPI signal), applied first - before
    # negation/rotation - since it's meant to pull faint signal out of the
    # raw scan
    if ENHANCE_IMG1:
    ...
    # Local adaptive contrast enhancement (CLAHE), applied after brightness
    # enhancement, before negation/rotation
    if CLAHE_IMG1:
        img1 = apply_clahe(img1, clip_limit=CLAHE_CLIP_LIMIT, tile_grid_size=CLAHE_TILE_GRID_SIZE)
        print(f"[1] Applied CLAHE to image 1 (clip_limit={CLAHE_CLIP_LIMIT}, tile_grid_size={CLAHE_TILE_GRID_SIZE})")
    if CLAHE_IMG2:
        img2 = apply_clahe(img2, clip_limit=CLAHE_CLIP_LIMIT, tile_grid_size=CLAHE_TILE_GRID_SIZE)
        print(f"[1] Applied CLAHE to image 2 (clip_limit={CLAHE_CLIP_LIMIT}, tile_grid_size={CLAHE_TILE_GRID_SIZE})")

    # Negate intensities where configured (e.g. DAPI -> white background,
    # black nuclei; leave H&E untouched)
    if NEGATE_IMG1:
